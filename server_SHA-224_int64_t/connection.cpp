...
int Connection::conn(const Params* p){
    ...
    int s = socket(AF_INET, SOCK_STREAM, 0);
        ...
        self_addr->sin_addr.s_addr = inet_addr(p->Address.c_str()); //Адрес
    ...
    string client_login = received_message.substr(0, 4); // "user"
    string client_salt = received_message.substr(4, 16); // "F80621F5AC05F1AA" 
    string client_hash = received_message.substr(20);    // хеш пароля
    ...
    if (sent_bytes == -1) {
        string errorMsg = "Ошибка send (соль): " + std::string(strerror(errno));
        logError(p->logFile, errorMsg);
        close(client_socket);
        close(s);
        throw std::system_error(errno, std::generic_category());
    ...
    */
    ...
    string hash = auth(client_salt, pass);
